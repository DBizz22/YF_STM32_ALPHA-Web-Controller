...
void uartINT_RX(uint8_t ch,uint32_t baud)
{
    ...
    switch (ch) 
    {
        case USART1_CH :    NVIC_EnableIRQ(USART1_IRQn);
                            break;
        case USART2_CH :    NVIC_EnableIRQ(USART2_IRQn);
                            break;
        case USART3_CH :    NVIC_EnableIRQ(USART3_IRQn);
    }
    __enable_irq();
}
...
{
    while((usart->SR&(0x01<<7))==0x00);     //wait for tansmit data register to be empty
    usart->DR=chr;
    while((usart->SR&(0x01<<6))==0x00);     //wait to complete transmission
...
char uartCHAR_RX(uint8_t ch)
{
    USART_TypeDef* usart=getUSART(ch);
    char temp;
    while((usart->SR&(0x01<<5))==0x00);     //wait to recieve data
    temp=usart->DR;
    return temp;
}
